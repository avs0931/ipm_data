...
class data_table:
    ...
    def get_name(self, use_prefix: bool = True):
        return self.ns_prefix + "_" + self.name if self.ns_prefix and use_prefix else self.name

    def append_column(self, csv_line: str):
        # Parse csv-line and create column definition for it
        c = data_column(table_name=self.get_name(), ns_prefix=self.ns_prefix, csv_line=csv_line)
    ...
    def drop_table_stmt(self, force_comment: bool = True):
        ...
        return f"{s}drop table if exists {self.get_name()} cascade;"

    def truncate_stmt(self, force_comment: bool = True):
        # Create TRUNCATE TABLE Statement
        s = "-- " if force_comment else ""
        return f"{s}truncate table {self.get_name()};"
    ...
    def create_stmt(self, indent: int = 4):
        ...
        # Construct statement
        stmt = list()
        stmt.append(f"create table {self.get_name()} (")
        # Apply lists to definition
        stmt.append(("\n" + " " * indent + ", ").join(cld))
        stmt.append(");")
        # Apply comments for table
        stmt.append(f"comment on table {self.get_name()} is '{self.comment}';" if self.comment else "")
    ...
    def copy_stmt(self, data_file: str):
        stmt = list()
        stmt.append(f"copy {self.get_name()}")
